...
allstates = sorted(allstates, key=lambda l: (len(l),l))
#print(startstate)
...
for state in allstates:
	...
	if(state==[]):
		...
		print("State   ",state)
		for letter in nfa["letters"]:
			uniquestatereached=set()
			print(letter)
			for eachstate in state:
				if (eachstate,letter) in nfatransfun:
					print(nfatransfun[(eachstate,letter)])
					val  = set(nfatransfun[(eachstate,letter)])
					print(val)
					uniquestatereached = uniquestatereached.union(val)
						#print(opchar)
			...
			print(list(uniquestatereached))
			dfatransfun[(tuple(state),letter)] =  [x for x in uniquestatereached]
			print(dfatransfun[(tuple(state),letter)])
#print(dfatransfun)
#print("**********************************")
dfatfinal=[]
